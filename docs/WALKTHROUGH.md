# maps-os: Standalone Usage Walkthrough

This is a human-facing guide. If you're an agent, see `AGENT_GUIDE.md`.
...
No mouse. Vim-style navigation. One key per action.
...
| `y` | Sync — flush local store to garden |
| `?` / `h` | Help |
| `q` | Quit |
...
okay so emma texted and i don't know how to feel about it
like on one hand it's what i wanted but on the other
i've been here before with her and it didn't work out
...
```bash
maps flash "emma"
maps flash "still thinking about the conversation with lior"
maps flash "the bit about the dog"
```

Logged as `FLASH: date | text`. Used by the intrusive loop arc — if the same name or topic appears 3+ times in recent flashes, it surfaces: *"'emma' has come up 4 times. That's a loop. What's underneath it?"*
...
When garden goes down, entries write locally. When garden returns:
...
The system never loses data. You don't need to do anything special when garden goes down — just keep logging. `sync` handles the rest.

---

### eval

Shows cassette's maps-os performance trend from passive RL history. Not surfaced automatically — explicit call only.
...
**`planning_hyperfocus`** — ADHD planning trap. Planning to plan. "What's the smallest first step on THE Thing?"
...
entries log to garden by default:

```bash
garden remember 'STATE: 2026-04-09 | thriving | ...' --graph cassette
```

If garden is unavailable: silent fallback to `~/.maps_os_local.db`. Sync when garden returns.

To check garden status:

```bash
maps sync --status
```

To force CLI to use specific graph:

```bash
maps vent "rough day" --graph cassette
```
...
→ email lior about the lease situation
→ check the error in polycule before end of day
```
...
## Migrating from life-os

If you have existing life-os data (MOOD/ENERGY/HABIT schema):

```bash
python3 scripts/migrate_legacy.py --dry-run   # preview what changes
python3 scripts/migrate_legacy.py             # migrate
```

Original values are preserved in `legacy_MOOD`, `legacy_ENERGY` etc. fields.
